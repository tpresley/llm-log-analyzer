...
List the JSON objects in a top level array wrapped in a \`\`\`json markdown block.

The JSON you output will be parsed, so never include comments or ellipses or any other values inside the JSON that could break parsing. I will get fired and become homeless if the JSON doesn't parse, so please be careful!
`;